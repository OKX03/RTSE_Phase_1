...
shared_data = {
    ...
    'debug_tokens': [],       
    'net_lane_position': 0,
    'low_light': False        # Challenge 1 flag ONLY
}
...
# Optimized Tapping: Shorter holds for faster mid-course corrections
TAP_HOLD_FRAMES = 8         
COOLDOWN_FRAMES = 8         
...
# ---------------------------------------------------------
# Perception & Decision (Ablation Version - No Police/Chasing)
# ---------------------------------------------------------
ROI_START_Y = 100
...
def detect_environment(front_frame):
    ...
    # Challenge 1: Peripheral Baseline Brightness Detection
    ...
    # Color Masks
    mask_green = cv2.inRange(roi_hsv, np.array([35, 45, 45]), np.array([85, 255, 255]))
    mask_red1 = cv2.inRange(roi_hsv, np.array([0, 110, 70]), np.array([10, 255, 255]))
    mask_red2 = cv2.inRange(roi_hsv, np.array([170, 110, 70]), np.array([180, 255, 255]))
        ...
        if cv2.contourArea(c) > MIN_AREA:
            ...
            if 0.3 < float(w)/h < 3.0:
                ...
                if lanes:
                    ...
                    detected_objects.append({'type': 'DANGER', 'color': 'RED', 'lanes': lanes, 'dist': dist})
    ...
    for c in contours_yellow:
        if cv2.contourArea(c) > MIN_AREA:
            ...
            if 0.3 < float(w)/h < 3.0:
                ...
                if lanes:
                    dist = (y + h/2 + ROI_START_Y) - 80
                    detected_objects.append({'type': 'DANGER', 'color': 'YELLOW', 'lanes': lanes, 'dist': dist})
...
def evaluate_decision(detected_objects, current_lane, low_light_mode):
    ...
    danger_dist = {-1: -1, 0: -1, 1: -1}
    target_dist = {-1: -1, 0: -1, 1: -1}

    for obj in detected_objects:
        if obj['type'] == 'DANGER':
            for lane in obj['lanes']: 
                danger_dist[lane] = max(danger_dist[lane], obj['dist'])
        elif obj['type'] == 'GREEN':
            for lane in obj['lanes']: 
                target_dist[lane] = max(target_dist[lane], obj['dist'])

    # Highest Priority: Evade Danger directly ahead (Only if it's close enough)
    DANGER_THRESHOLD = 20

    if danger_dist[0] > DANGER_THRESHOLD:
        dist_L = danger_dist[-1]
        dist_R = danger_dist[1]
        
        if dist_R < dist_L:
            target_steer = 1.0
            debug_text = "EVADE RIGHT (Safest)"
        elif dist_L < dist_R:
            target_steer = -1.0
            debug_text = "<< EVADE LEFT (Safest)"
        else:
            if dist_L == -1:
                target_steer = 1.0
    ...
    # Medium Priority: Seek Green Target if safe
    if target_dist[0] > 0:
        target_steer = 0.0
    ...
    best_side = None
    if target_dist[-1] > target_dist[1]: best_side = -1
    elif target_dist[1] > target_dist[-1]: best_side = 1

    if best_side == -1:
        if danger_dist[-1] == -1 or target_dist[-1] > danger_dist[-1] + 15:
            target_steer = -1.0
            debug_text = "<< SEEK GREEN LEFT"
        else: debug_text = "ABORT TARGET (DANGER LEFT)"
        return target_steer, target_accel, debug_text
        ...
        else: debug_text = "ABORT TARGET (DANGER RIGHT)"
        return target_steer, target_accel, debug_text
...
def processing_task():
    ...
    if front_frame is not None:
        detected_objects, debug_tokens, low_light = detect_environment(front_frame)
        target_steer, target_accel, debug_text = evaluate_decision(detected_objects, current_lane, low_light)
            
        full_debug_text = f"[{current_lane}] {debug_text} | Steer: {target_steer:.1f}"

        with data_lock:
            shared_data['steering_input'] = target_steer
            shared_data['acceleration_input'] = target_accel
            shared_data['debug_tokens'] = debug_tokens
            shared_data['debug_info'] = full_debug_text
            shared_data['low_light'] = low_light
    ...
    try:
        while is_running:
            ...
            if front_frame is not None and not display_paused:
                ...
                # Active UI display
                cv2.putText(display_front, debug_info, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
