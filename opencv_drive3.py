...
# RTOS Message Queues (Mailboxes)
frame_queue = queue.Queue(maxsize=1)
back_frame_queue = queue.Queue(maxsize=1)
control_queue = queue.Queue(maxsize=1)
ui_queue = queue.Queue(maxsize=1)

# Persistent Global States (Atomic / GIL-protected)
net_lane_position = 0
yellow_effect_timer = 0
yellow_effect_type = 0
is_running = True
...
def read_single_camera(sock, target_queue):
    ...
    try:
        ...
        if len(received_bytes) == image_length:
            ...
            if frame is not None:
                ...
                try: target_queue.put_nowait(frame)
...
def read_front_camera_task(): read_single_camera(front_camera_sock, frame_queue)
def read_back_camera_task(): read_single_camera(back_camera_sock, back_frame_queue)
...
def detect_environment(front_frame, yellow_effect_type):
    """
    PERCEPTION MODULE: 
    ...
    # 图像损坏/延迟/隐藏效果：致盲 (返回空检测)
    if yellow_effect_type in [1, 3, 5]:
        return detected_objects, debug_tokens, is_desperate, False
        ...
        if area > 5:
            ...
            if 0.3 < float(w)/h < 3.0:
                ...
                if lanes:
                    ...
                    # 碰撞判定：如果在0车道且中心极度贴近正中，判定为触碰黄币
                    actual_y = y + h/2 + ROI_START_Y
                    cx = x + w/2
                    if abs(cx - 160) < 25 and (actual_y > 180):
                        yellow_detected = True

    # 灰化效果：所有token都变成灰色danger
    if yellow_effect_type == 2:
        for c in contours_g:
            area = cv2.contourArea(c)
            if area > 5:
                x, y, w, h = cv2.boundingRect(c)
                if 0.3 < float(w)/h < 3.0:
                    ...
                    if lanes:
                        dist = (y + h/2 + ROI_START_Y) - 80
                        # 绿币也视为危险灰色token
                        detected_objects.append({'type': 'DANGER', 'color': 'GRAY', 'priority': 2, 'lanes': lanes, 'area': area, 'dist': dist})
                        debug_tokens.append(('DANGER_GRAY', x*2, (y+ROI_START_Y)*2, w*2, h*2))
    else:
        # 正常情况：检测绿币
        for c in contours_g:
            area = cv2.contourArea(c)
    ...
    return detected_objects, debug_tokens, is_desperate, yellow_detected
...
class TemporalMemory:
    def __init__(self):
        self.dangers = {-1: -1, 0: -1, 1: -1}
        self.greens = {-1: -1, 0: -1, 1: -1}
        
    def update(self, detected_objects, is_blind):
        # 1. Apply Temporal Extrapolation (Simulate objects moving towards car)
        for lane in [-1, 0, 1]:
            if self.dangers[lane] > 0: self.dangers[lane] += 5 
            if self.greens[lane] > 0: self.greens[lane] += 5
            
        # 2. Update with new observations using EMA (if not blind)
        if not is_blind:
            curr_dangers = {-1: -1, 0: -1, 1: -1}
            curr_greens = {-1: -1, 0: -1, 1: -1}
            
            for obj in detected_objects:
                for lane in obj['lanes']:
                    if obj['type'] == 'DANGER': curr_dangers[lane] = max(curr_dangers[lane], obj['dist'])
                    elif obj['type'] == 'GREEN': curr_greens[lane] = max(curr_greens[lane], obj['dist'])
            
            for lane in [-1, 0, 1]:
                if curr_dangers[lane] > 0:
                    if self.dangers[lane] > 0: self.dangers[lane] = 0.7 * curr_dangers[lane] + 0.3 * self.dangers[lane]
                    else: self.dangers[lane] = curr_dangers[lane]
                if curr_greens[lane] > 0:
                    if self.greens[lane] > 0: self.greens[lane] = 0.7 * curr_greens[lane] + 0.3 * self.greens[lane]
                    else: self.greens[lane] = curr_greens[lane]
                    
    def get_extrapolated_objects(self):
        objs = []
        for lane in [-1, 0, 1]:
            if 0 < self.dangers[lane] < 200:
                objs.append({'type': 'DANGER', 'lanes': [lane], 'dist': self.dangers[lane], 'priority': 2})
            if 0 < self.greens[lane] < 200:
                objs.append({'type': 'GREEN', 'lanes': [lane], 'dist': self.greens[lane]})
        return objs

temporal_memory = TemporalMemory()

def processing_task():
    global net_lane_position, yellow_effect_timer, yellow_effect_type
    
    try: front_frame = frame_queue.get(timeout=0.01)
    except queue.Empty: return
        
    detected_objects, debug_tokens, is_desperate, yellow_detected = detect_environment(front_frame, yellow_effect_type)
    
    if yellow_detected:
        yellow_effect_type = random.choice([1, 2, 3, 4, 5])
        yellow_effect_timer = 200 
    elif yellow_effect_timer > 0:
        yellow_effect_timer -= 1
    else:
        yellow_effect_type = 0
        
    is_blind = yellow_effect_type in [1, 3, 5]
    temporal_memory.update(detected_objects, is_blind)
    
    if is_blind:
        detected_objects = temporal_memory.get_extrapolated_objects()
        
    target_steer, debug_text = evaluate_decision(detected_objects, net_lane_position, is_desperate)
    
    if yellow_effect_type > 0:
        debug_text = f"[EFX {yellow_effect_type}] " + debug_text

    control_data = {'steering_input': target_steer, 'acceleration_input': 1.0, 'yellow_effect_type': yellow_effect_type}
    try: control_queue.get_nowait()
    except queue.Empty: pass
    try: control_queue.put_nowait(control_data)
    except queue.Full: pass
    
    ui_data = {'front_frame': front_frame, 'debug_info': debug_text, 'debug_tokens': debug_tokens, 'is_desperate': is_desperate, 'steer_input': target_steer}
    try: ui_queue.get_nowait()
    except queue.Empty: pass
    try: ui_queue.put_nowait(ui_data)
    except queue.Full: pass
            
def send_controls_task():
    global control_conn, tap_state, tap_timer, active_steering_value, net_lane_position
    if control_conn is None: return
    
    try: control_data = control_queue.get(timeout=0.01)
    except queue.Empty: return
        
    desired_steer = control_data['steering_input']
    accel_input = control_data['acceleration_input']
    eff_type = control_data['yellow_effect_type']
    ...
    if tap_state == 'IDLE':
        if desired_steer != 0.0:
            ...
            if desired_steer < -0.1: net_lane_position = max(-1, net_lane_position - 1)
            elif desired_steer > 0.1: net_lane_position = min(1, net_lane_position + 1)
    ...
    try:
        while is_running:
            try:
                ...
                front_frame = ui_data['front_frame']
                debug_info = ui_data['debug_info']
                debug_tokens = ui_data['debug_tokens']
                is_desperate_flag = ui_data['is_desperate']
                steer_input = ui_data['steer_input']
            except queue.Empty:
                pass
                
            try: back_frame = back_frame_queue.get_nowait()
            except queue.Empty: back_frame = None
            ...
            if front_frame is not None and not display_paused:
                ...
                for token_data in debug_tokens:
                    if len(token_data) >= 5:
                        ...
                        if 'RED' in ttype: color = (0, 0, 255)
                        elif 'YELLOW' in ttype: color = (0, 255, 255)
                        else: color = (0, 255, 0)
